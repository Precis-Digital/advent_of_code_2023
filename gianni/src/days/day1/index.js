...
import('./class.js').then((module) => {
  ...
  const { data: linesPart1 } = get.dataAndSize('part1-data.txt', day);
  const { data: linesPart2 } = get.dataAndSize('part2-data.txt', day);
  ...
  const solution = new DayClass(dataLines, false);
  ...
  log.execute(DayClass, 1, linesPart1);
  log.execute(DayClass, 2, linesPart2);
});
